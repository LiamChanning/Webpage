...
// plugins
Matter.use(MatterAttractors); // constants
...
var BUMPER_BOUNCE = 1;
var PADDLE_PULL = 0.002; // score elements

var currentScoreEl = document.querySelector('.current-score span');
var highScoreEl = document.querySelector('.high-score span'); // shared variables

var currentScore, highScore;
var engine, world, render, pinball, stopperGroup;
var leftPaddle, leftUpStopper, leftDownStopper, isLeftPaddleUp;
var rightPaddle, rightUpStopper, rightDownStopper, isRightPaddleUp;

function init() {
  // engine (shared)
  engine = Matter.Engine.create(); // world (shared)
  ...
  Matter.Render.run(render); // runner

  var runner = Matter.Runner.create();
  Matter.Runner.run(runner, engine); // used for collision filtering on various bodies

  stopperGroup = Matter.Body.nextGroup(true); // starting values
...
function createStaticBodies() {
  Matter.World.add(world, [// table boundaries (top, bottom, left, right)
  boundary(250, -30, 500, 100), boundary(250, 830, 500, 100), boundary(-30, 400, 100, 800), boundary(530, 400, 100, 800), // dome
  path(250, 90, PATHS.DOME), // pegs (left, mid, right)
  wall(140, 140, 20, 40, COLOR.INNER), wall(225, 140, 20, 40, COLOR.INNER), wall(310, 140, 20, 40, COLOR.INNER), // top bumpers (left, mid, right)
  bumper(105, 250), bumper(225, 250), bumper(345, 250), // bottom bumpers (left, right)
  bumper(165, 340), bumper(285, 340), // shooter lane wall
  wall(440, 520, 20, 560, COLOR.OUTER), // drops (left, right)
  path(25, 360, PATHS.DROP_LEFT), path(425, 360, PATHS.DROP_RIGHT), // slingshots (left, right)
  wall(120, 510, 20, 120, COLOR.INNER), wall(330, 510, 20, 120, COLOR.INNER), // out lane walls (left, right)
  wall(60, 529, 20, 160, COLOR.INNER), wall(390, 529, 20, 160, COLOR.INNER), // flipper walls (left, right);
  wall(93, 624, 20, 98, COLOR.INNER, -0.96), wall(357, 624, 20, 98, COLOR.INNER, 0.96), // aprons (left, right)
  path(79, 740, PATHS.APRON_LEFT), path(371, 740, PATHS.APRON_RIGHT), // reset zones (center, right)
  reset(225, 50), reset(465, 30)]);
}
...
function createPaddles() {
  ...
  Matter.World.add(world, [leftUpStopper, leftDownStopper, rightUpStopper, rightDownStopper]); // this group lets paddle pieces overlap each other

  var paddleGroup = Matter.Body.nextGroup(true); // Left paddle mechanism
  ...
  Object.values(paddleLeft).forEach(function (piece) {
  ...
  Object.values(paddleRight).forEach(function (piece) {
    piece.collisionFilter.group = paddleGroup;
  });
...
function createEvents() {
  ...
  Matter.Events.on(engine, 'collisionStart', function (event) {
    event.pairs.filter(function (pair) {
...
function pingBumper(bumper) {
  updateScore(currentScore + 10); // flash color
...
function bumper(x, y) {
  var bumper = Matter.Bodies.circle(x, y, 25, {
    label: 'bumper',
    isStatic: true,
    render: {
...
function stopper(x, y, side, position) {
  ...
  var attracteeLabel = side === 'left' ? 'paddleLeftComp' : 'paddleRightComp';
  return Matter.Bodies.circle(x, y, 40, {
    ...
    plugin: {
      attractors: [// stopper is always a, other body is b
      function (a, b) {
        if (b.label === attracteeLabel) {
          var isPaddleUp = side === 'left' ? isLeftPaddleUp : isRightPaddleUp;
          var isPullingUp = position === 'up' && isPaddleUp;
          var isPullingDown = position === 'down' && !isPaddleUp;
